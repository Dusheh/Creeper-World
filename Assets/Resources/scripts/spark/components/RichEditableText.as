package spark.components
{
   ...
   public class RichEditableText extends UIComponent implements IFocusManagerComponent, IIMESupport, ISystemCursorClient, IViewport, IEditableText
   {
      ...
      {
         this.unbounded = new Rectangle(0,0,NaN,NaN);
         super();
         initClass();
         this.text = "";
      ...
      override protected function commitProperties() : void
      {
         ...
         if(this.textChanged)
         {
            if(false)
            {
               this.preserveSelectionOnSetText = true;
            }
         ...
         if(this.displayAsPasswordChanged)
         {
            ...
            if(this.displayAsPassword)
            {
               ...
               _loc3_ = StringUtil.repeat(this.passwordChar,this._text.length);
         ...
         if(this.preserveSelectionOnSetText)
         {
            this.preserveSelectionOnSetText = false;
            if(_loc1_ != -1)
            {
      ...
      override protected function measure() : void
      {
            ...
            if(this.typicalText)
            {
               ...
               if(this._typicalTextFlow)
               {
                  if(!staticTextFlowFactory)
                  {
                     ...
                     staticTextFlowFactory.compositionBounds = this.unbounded;
                  }
                  if(this._typicalTextFlow.flowComposer)
                  {
                     this._typicalTextFlow.flowComposer.swfContext = ISWFContext(this.embeddedFontContext);
                  }
               ...
               else
               {
                  if(!staticStringFactory)
                  {
                     ...
                     staticStringFactory.compositionBounds = this.unbounded;
                  }
         ...
         else
         {
            ...
            if(!isNaN(this.widthConstraint) || !isNaN(explicitWidth) || !isNaN(this.widthInChars))
            {
               ...
               {
                  measuredWidth = this.textContainerManager.compositionWidth;
               }
               else
               {
                  measuredWidth = Math.ceil(_loc1_.width);
            ...
            if(this.autoSize && getStyle("lineBreak") == "toFit")
            {
      ...
      override protected function updateDisplayList(param1:Number, param2:Number) : void
      {
         ...
         this.updateStylesIfChanged();
         if(this.autoSize && this.lastUnscaledHeight == param2 && this.lastUnscaledWidth == param1 && this.remeasureText(param1,param2))
         {
         ...
         if(this.clipAndEnableScrolling && this._textContainerManager.composeState != TextContainerManager.COMPOSE_COMPOSER)
         {
            this._textContainerManager.convertToTextFlowWithComposer();
         }
         if(this.autoSize && !isNaN(this.lastUnscaledWidth) && (this.lastUnscaledWidth != param1 || this.lastUnscaledHeight != param2))
         {
            if(this._textContainerManager.composeState == TextContainerManager.COMPOSE_COMPOSER)
            {
               this._textContainerManager.getTextFlow().flowComposer.getControllerAt(0).shapesInvalid = true;
            }
      ...
      {
         if(this.editingMode != EditingMode.READ_ONLY && this._textContainerManager.composeState != TextContainerManager.COMPOSE_COMPOSER)
         {
            this._textContainerManager.beginInteraction();
            this._textContainerManager.endInteraction();
         }
      ...
      override public function drawFocus(param1:Boolean) : void
      {
         ...
         {
            _loc2_ = focusManager;
            if(_loc2_ && _loc2_.getFocus() != this)
            {
               return;
            }
         }
      ...
      public function getHorizontalScrollPositionDelta(param1:uint) : Number
      {
         ...
         switch(param1)
         {
            case NavigationUnit.LEFT:
               return _loc2_.left <= 0 ? 0 : Number(Math.max(_loc4_,-_loc5_));
            case NavigationUnit.RIGHT:
               return _loc2_.right >= this.contentWidth ? 0 : Number(Math.min(_loc3_,_loc5_));
      ...
      {
         validateProperties();
         this.textContainerManager.scrollToRange(param1,param2);
      }
      ...
      public function getFormatOfRange(param1:Vector.<String> = null, param2:int = -1, param3:int = -1) : TextLayoutFormat
      {
         ...
         validateProperties();
         var _loc5_:Object = TextLayoutFormat.description;
         ...
         if(_loc8_)
         {
            _loc11_ = this._textContainerManager.getCommonContainerFormat();
         }
         if(_loc9_)
         {
            _loc12_ = this._textContainerManager.getCommonParagraphFormat(param2,param3);
         }
         if(_loc10_)
         {
            _loc13_ = this._textContainerManager.getCommonCharacterFormat(param2,param3);
         }
         for each(_loc6_ in param1)
         {
            if(_loc6_ in _loc5_)
            {
               if((_loc7_ = _loc5_[_loc6_].category) == Category.CONTAINER && _loc11_)
               {
                  _loc4_[_loc6_] = _loc11_[_loc6_];
               }
               else if(_loc7_ == Category.PARAGRAPH && _loc12_)
               {
                  _loc4_[_loc6_] = _loc12_[_loc6_];
               }
               else if(_loc7_ == Category.CHARACTER && _loc13_)
               {
      ...
      public function setFormatOfRange(param1:TextLayoutFormat, param2:int = -1, param3:int = -1) : void
      {
         ...
         var _loc9_:* = null;
         validateProperties();
         var _loc7_:Object = TextLayoutFormat.description;
         ...
         this._textContainerManager.applyFormatOperation(_loc6_,_loc5_,_loc4_,param2,param3);
      }
      ...
      private function calculateFontMetrics() : void
      {
         ...
         if(_loc6_ = ISWFContext(this.embeddedFontContext))
         {
      ...
      private function calculateHeightInLines() : Number
      {
         ...
         return Number(_loc1_ + this.descent);
      }
      ...
      private function handleInsertText(param1:String, param2:Boolean = false) : void
      {
         ...
         var _loc3_:Boolean = this._textContainerManager.insertTextOperation(param1,this._selectionAnchorPosition,this._selectionActivePosition);
         if(_loc3_)
         {
            dispatchEvent(new FlexEvent(FlexEvent.VALUE_COMMIT));
         }
      ...
      {
         var _loc2_:Boolean = this.restrict || this.maxChars || this.displayAsPassword;
      ...
      mx_internal function focusInHandler(param1:FocusEvent) : void
      {
         ...
         if(_loc2_ && this.editingMode == EditingMode.READ_WRITE)
         {
         ...
         if(focusManager && this.multiline)
         {
      ...
      mx_internal function focusOutHandler(param1:FocusEvent) : void
      {
         ...
         if(this.clearUndoOnFocusOut && this.undoManager)
         {
      ...
      private function textContainerManager_compositionCompleteHandler(param1:CompositionCompleteEvent) : void
      {
         ...
         if(this._textFlow && this.clipAndEnableScrolling)
         {
      ...
      private function textContainerManager_damageHandler(param1:DamageEvent) : void
      {
         ...
         if(this._textFlow && this._textFlow.generation == this.lastGeneration)
         {
            return;
         }
         if(this.textChanged || this.textFlowChanged || this.contentChanged)
      ...
      private function textContainerManager_flowOperationBeginHandler(param1:FlowOperationEvent) : void
      {
         ...
         if(_loc2_ is InsertTextOperation)
         {
            ...
            _loc4_ = _loc3_.text;
            _loc6_ = (_loc5_ = _loc3_.deleteSelectionState) == null ? 0 : int(_loc5_.absoluteEnd - _loc5_.absoluteStart);
            ...
            if(this._displayAsPassword)
            {
               ...
               if(_loc4_.length > 0)
               {
                  ...
                  _loc4_ = StringUtil.repeat(this.passwordChar,_loc4_.length);
               }
            }
            _loc3_.text = _loc4_;
         }
