package com.wbwar.creeper.screens
{
   ...
   public class SubmitScoreScreen extends Sprite
   {
      ...
      public function SubmitScoreScreen()
      {
         ...
         var nameTitle:TextField = Text.text("Your Player Name",10,16777215);
         nameTitle.x = 10;
         nameTitle.y = 9;
         ...
         this.systemTitle = Text.text("---",16,12632256);
      ...
      private function onSubmitClick(param1:Event) : void
      {
         ...
         GameData.save();
         Score.submitScoreOld(_loc2_,_loc3_,this.gameType,this.submitCompleteOld,this.errorCompleteOld);
         var _loc4_:String = this.gameType + "-" + this.missionNumber;
         Score.submitScore(_loc4_,_loc2_,_loc3_,this.systemScore,this.systemPlayCount,this.submitComplete,this.errorComplete);
      }
      ...
      public function show(param1:Boolean, param2:Boolean, param3:int, param4:int, param5:int, param6:int, param7:int) : void
      {
         if(param1)
         {
            this.gameType = Score.GAMETYPE_STORY;
         }
         else if(param2)
         {
            this.gameType = Score.GAMETYPE_SPECIAL;
         }
         else
         {
            this.gameType = Score.GAMETYPE_CONQUEST;
         }
         if(false)
         {
         ...
         if(param1)
         {
            this.systemTitle.text = MissionSelection.mws[param7].title.text + " Score";
            this.table = "story";
         }
         else if(param2)
         {
            this.systemTitle.text = SpecialMissionSelection.missionWorlds[param7].title.text + " Score";
            this.table = "special";
         }
         else
         {
            this.systemTitle.text = FreeplayMissionSelection.systemNames[int(param7 / 5)] + " " + (param7 % 5 + 1) + " Score";
            this.table = "conquest";
         }
         this.missionNumber = param7;
         this.systemTitle.x = this.submitSprite.width / 2 - this.systemTitle.width / 2;
         this.systemScore = param5;
         this.systemPlayCount = param6;
         this.systemScoreText.text = String(param5);
         this.systemPlayCountText.text = "# Plays: " + String(param6);
         this.systemScoreText.x = SUBMITSPRITE_WIDTH / 2 - this.systemScoreText.width / 2;
      ...
      {
         if(this.gameType == Score.GAMETYPE_STORY && this.missionNumber == 19)
         {
            alpha = 0;
            visible = false;
