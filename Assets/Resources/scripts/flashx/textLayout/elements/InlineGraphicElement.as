package flashx.textLayout.elements
{
   ...
   public final class InlineGraphicElement extends FlowLeafElement
   {
      ...
      tlf_internal function elementWidthWithMarginsAndPadding() : Number
      {
         ...
         var _loc2_:Number = _loc1_.computedFormat.blockProgression == BlockProgression.RL ? Number(getEffectivePaddingTop() + getEffectivePaddingBottom()) : Number(getEffectivePaddingLeft() + getEffectivePaddingRight());
      ...
      tlf_internal function elementHeightWithMarginsAndPadding() : Number
      {
         ...
         var _loc2_:Number = _loc1_.computedFormat.blockProgression == BlockProgression.RL ? Number(getEffectivePaddingLeft() + getEffectivePaddingRight()) : Number(getEffectivePaddingTop() + getEffectivePaddingBottom());
      ...
      tlf_internal function set elementWidth(param1:Number) : void
      {
         ...
         if(_blockElement)
         {
            (_blockElement as GraphicElement).elementWidth = this.effectiveFloat != Float.NONE ? 0 : Number(this.elementWidthWithMarginsAndPadding());
         }
      ...
      tlf_internal function set elementHeight(param1:Number) : void
      {
         ...
         if(_blockElement)
         {
            (_blockElement as GraphicElement).elementHeight = this.effectiveFloat != Float.NONE ? 0 : Number(this.elementHeightWithMarginsAndPadding());
         }
         modelChanged(ModelChange.ELEMENT_MODIFIED,this,0,textLength,true,false);
      }
      ...
      private function internalSetWidth(param1:*) : void
      {
         ...
         this.elementWidth = !!this.widthIsComputed() ? 0 : Number(Number(this.internalWidth));
      ...
      private function internalSetHeight(param1:*) : void
      {
         ...
         this.elementHeight = !!this.heightIsComputed() ? 0 : Number(Number(this.internalHeight));
      ...
      tlf_internal function getTypographicAscent(param1:TextLine) : Number
      {
         ...
         return Number((_loc6_ += param1.getBaselinePosition(TextBaseline.ROMAN) - param1.getBaselinePosition(_loc5_)) + _loc4_.baselineShift);
      }
      ...
      override tlf_internal function getCSSInlineBox(param1:String, param2:TextLine, param3:ParagraphElement = null, param4:ISWFContext = null) : Rectangle
      {
         ...
         (_loc5_ = new Rectangle()).top = -this.getTypographicAscent(param2);
      ...
      override tlf_internal function calculateStrikeThrough(param1:TextLine, param2:String, param3:FontMetrics) : Number
      {
         ...
         if(_loc5_ = this._placeholderGraphic.parent)
         {
            ...
            {
               _loc4_ = Number(this.placeholderGraphic.parent.y + (this.elementHeight / 2 + Number(getEffectivePaddingTop())));
            }
            else
            {
               ...
               {
                  _loc4_ = Number(this._placeholderGraphic.parent.x - (this.elementHeight / 2 + _loc6_));
               }
               else
               {
                  _loc4_ = Number(this._placeholderGraphic.parent.x - (this.elementWidth / 2 + _loc6_));
               }
            }
         }
      ...
      override tlf_internal function calculateUnderlineOffset(param1:Number, param2:String, param3:FontMetrics, param4:TextLine) : Number
      {
         ...
         if(_loc7_ = this._placeholderGraphic.parent)
         {
            ...
            {
               _loc6_ = Number(_loc7_.y + this.elementHeightWithMarginsAndPadding());
            }
            else
            {
               if(_loc5_.computedFormat.locale.toLowerCase().indexOf("zh") == 0)
               {
                  return Number((_loc6_ = Number(_loc7_.x - this.elementHeightWithMarginsAndPadding())) - (param3.underlineOffset + param3.underlineThickness / 2));
               }
               _loc6_ = Number(_loc7_.x - getEffectivePaddingLeft());
            }
         }
         _loc6_ += param3.underlineOffset + param3.underlineThickness / 2;
         var _loc8_:String;
         if((_loc8_ = _loc5_.getEffectiveJustificationRule()) == JustificationRule.EAST_ASIAN)
         {
            _loc6_ += 1;
         }
         return _loc6_;
