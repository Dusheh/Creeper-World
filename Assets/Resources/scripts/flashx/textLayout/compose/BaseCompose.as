   ...
   public class BaseCompose
   {
      ...
      private function composeBlockElement(param1:FlowGroupElement, param2:int) : Boolean
      {
         ...
         while(_loc5_ < param1.numChildren && (param2 <= this._stopComposePos || !this.parcelList.atLast()))
         {
            ...
            {
               _loc7_ = _loc3_.getEffectivePaddingTop();
               _loc8_ = _loc3_.getEffectivePaddingBottom();
               _loc9_ = _loc3_.getEffectivePaddingRight();
               _loc10_ = _loc3_.getEffectivePaddingLeft();
            }
            else
            {
               _loc7_ = _loc3_.getEffectivePaddingLeft();
               _loc8_ = _loc3_.getEffectivePaddingRight();
               _loc9_ = _loc3_.getEffectivePaddingTop();
               _loc10_ = _loc3_.getEffectivePaddingBottom();
            }
      ...
      private function advanceToComposeStartPosition() : Boolean
      {
         ...
         if(_loc2_.controller.numFloats)
         {
            if(this._measuring)
            {
               return false;
         ...
         this._curLineLeadingModel = _loc3_.getParagraph().getEffectiveLeadingModel();
         var _loc4_:FlowLeafElement;
         var _loc5_:int = (_loc4_ = this._textFlow.findLeaf(this._curLine.absoluteStart)).getAbsoluteStart();
         this.calculateLeadingParameters(_loc4_,_loc5_,TextFlowLine.findNumberLine(this._curLine.getTextLine()));
         if(this._startComposePosition == this._curElement.getParagraph().getAbsoluteStart())
         {
            this._previousLine = null;
         }
         ...
         var _loc6_:int = this._curLine.columnIndex == -1 ? 0 : int(this._curLine.columnIndex);
         ...
         while(_loc8_ < _loc6_)
         {
            ...
            if(_loc10_ = this._curParcel.controller.numFloats)
            {
               while(_loc7_ < _loc10_)
               {
                  if((_loc11_ = this._curParcel.controller.getFloatAt(_loc7_)).columnIndex > this._curParcel.columnIndex)
                  {
                     break;
                  }
                  if(_loc11_.floatType != Float.NONE && _loc11_.absolutePosition < this._startComposePosition)
                  {
                     ...
                     _loc13_ = this._blockProgression == BlockProgression.RL ? Number(_loc12_.elementWidthWithMarginsAndPadding()) : Number(_loc12_.elementHeightWithMarginsAndPadding());
      ...
      tlf_internal function computeTextFlowLineMinimumLogicalTop(param1:TextFlowLine, param2:TextLine) : Number
      {
         ...
         if(param1.hasGraphicElement)
         {
            ...
            _loc6_ = this._blockProgression == BlockProgression.RL ? Number(param1.x + _loc5_) : Number(param1.y + param1.ascent - _loc5_);
            ...
            if(_loc7_.numFloats > 0)
            {
               while(_loc3_ < _loc8_)
               {
                  if(!(_loc9_ = _loc7_.getFloatAtPosition(_loc3_)))
                  {
                     break;
                  }
      ...
      protected function composeParagraphElement(param1:ParagraphElement, param2:int) : Boolean
      {
         ...
         {
            _loc6_ = (_loc5_ = param1.getTextBlock()).lastLine;
            while(_loc6_ && _loc4_)
            {
               if(_loc6_.parent)
               {
      ...
      protected function composeParagraphElementIntoLines() : Boolean
      {
         ...
         while(_loc1_)
         {
            ...
            this._textIndent = this._curElementStart + this._curElementOffset <= _loc6_ ? Number(this._curParaFormat.textIndent) : Number(0);
            if(this._parcelList.atEnd())
            {
               _loc1_ = false;
               break;
            ...
            _loc9_ = (_loc8_ = TextFlowLine.findNumberLine(_loc2_)) && TextFlowLine.getNumberLineListStylePosition(_loc8_) == ListStylePosition.OUTSIDE || _loc7_ == TextAlign.CENTER || _loc7_ == TextAlign.RIGHT;
            if(false)
            {
               if(_loc2_["hasTabs"])
               {
            ...
            _loc11_ = this._atColumnStart && this._curParaFormat.leadingModel != LeadingModel.BOX ? 0 : Number(this._curLine.spaceBefore);
            _loc12_ = !!this._atColumnStart ? 0 : Number(this._paragraphSpaceCarried);
      ...
      {
         var _loc3_:Number = this._curParaFormat.direction == Direction.LTR ? Number(this._lineSlug.leftMargin) : Number(this._lineSlug.rightMargin);
         var _loc4_:* = null;
      ...
      protected function preProcessILGs(param1:int) : Boolean
      {
         ...
         while(_loc4_)
         {
            ...
            {
               (_loc5_ = _loc4_ as InlineGraphicElement).setEffectiveFloat(!!this._forceILGs ? "null" : _loc5_.computedFloat);
               _loc2_ = true;
            }
      ...
      protected function processFloatsAtLineEnd(param1:TextLine) : Boolean
      {
         ...
         if(this._pushInFloats && this._pushInFloats.length > 0)
         {
            ...
            if(this._curLine.absoluteStart + this._curLine.textLength <= _loc8_)
            {
               ...
               while(_loc9_ >= 0)
               {
                  ...
                  _loc11_ = this._blockProgression == BlockProgression.RL ? Number(_loc10_.elementWidth + _loc10_.getEffectivePaddingLeft() + _loc10_.getEffectivePaddingRight()) : Number(_loc10_.elementHeightWithMarginsAndPadding());
                  if((_loc12_ = this._curLine.controller.getFloatAtPosition(_loc8_)) && _loc12_.absolutePosition == _loc8_)
                  {
                     _loc13_ = !!isNaN(this._lastLineDescent) ? 0 : Number(this._lastLineDescent);
         ...
         while(_loc2_ < _loc4_)
         {
            ...
            {
               if((_loc14_ = InlineGraphicElement(_loc3_)).computedFloat == Float.NONE || this._forceILGs)
               {
                  _loc6_ = true;
               }
      ...
      protected function processInlinesAtLineEnd(param1:TextLine) : void
      {
         ...
         while(_loc2_ < _loc4_)
         {
            if(_loc3_ is InlineGraphicElement)
            {
               if((_loc5_ = _loc3_ as InlineGraphicElement).computedFloat == Float.NONE || this._forceILGs)
               {
      ...
      {
         var _loc3_:Number = this._blockProgression == BlockProgression.RL ? Number(-param1.getEffectivePaddingRight()) : Number(param1.getEffectivePaddingLeft());
         var _loc4_:Number = param1.getEffectivePaddingTop();
         var _loc5_:DisplayObject = param1.placeholderGraphic.parent;
         this._curParcel.controller.addFloatAt(this._curParaStart + param1.getElementRelativeStart(this._curParaElement),param1.graphic,Float.NONE,_loc3_,_loc4_,!!_loc5_ ? Number(_loc5_.alpha) : Number(1),!!_loc5_ ? _loc5_.transform.matrix : null,this._parcelList.totalDepth,0,this._curParcel.columnIndex,param2);
         return true;
      }
      ...
      protected function composeFloat(param1:InlineGraphicElement, param2:Boolean) : Boolean
      {
         ...
         {
            _loc4_ = Number(this._lastLineDescent);
         }
         var _loc5_:* = 0;
         if(this._curLine && this._curParaElement != this._curLine.paragraph && !this._atColumnStart)
         {
            _loc5_ = Number(Math.max(this._curParaElement.computedFormat.paragraphSpaceBefore,this._paragraphSpaceCarried));
         }
         ...
         if(_loc10_)
         {
            if((_loc11_ = param1.computedFloat) == Float.START)
            {
            ...
            _loc14_ = !!isNaN(this._lastLineDescent) ? 0 : Number(this._lastLineDescent);
      ...
      private function calculateFloatBounds(param1:InlineGraphicElement, param2:Boolean, param3:String) : Rectangle
      {
         ...
         if(param2)
         {
            _loc4_.x = this._curParcel.right - 0 - param1.elementWidth - param1.getEffectivePaddingRight();
            _loc4_.y = param3 == Float.LEFT ? Number(this._curParcel.y + _floatSlug.leftMargin + param1.getEffectivePaddingTop()) : Number(this._curParcel.bottom - 0 - param1.getEffectivePaddingBottom() - param1.elementHeight);
            _loc4_.width = param1.elementWidth;
            _loc4_.height = param1.elementHeight;
         }
         else
         {
            _loc4_.x = param3 == Float.LEFT ? Number(this._curParcel.x + _floatSlug.leftMargin + param1.getEffectivePaddingLeft()) : Number(this._curParcel.right - 0 - param1.getEffectivePaddingRight() - param1.elementWidth);
      ...
      private function calculateLineWidthExplicit(param1:TextLine) : Number
      {
         ...
         var _loc5_:Rectangle = param1.getAtomBounds(_loc3_ >= 0 ? int(_loc3_) : 0);
         var _loc6_:Number = this._blockProgression == BlockProgression.TB ? (_loc3_ >= 0 ? Number(_loc5_.right) : Number(_loc5_.left)) : (_loc3_ >= 0 ? Number(_loc5_.bottom) : Number(_loc5_.top));
         if(_loc2_)
         {
      ...
      private function getRightSideGap(param1:TextFlowLine, param2:Boolean) : Number
      {
         ...
         if(_loc4_.direction == Direction.RTL && param1.location & 0)
         {
            ...
            if(param1.hasNumberLine && _loc3_.getParentByType(ListItemElement).computedFormat.listStylePosition == ListStylePosition.INSIDE)
            {
               ...
               _loc5_ += TextFlowLine.getNumberLineInsideLineWidth(_loc7_);
            }
         }
         do
         {
            _loc5_ += this._blockProgression == BlockProgression.TB ? _loc3_.getEffectivePaddingRight() : _loc3_.getEffectivePaddingBottom();
      ...
      private function getLeftSideGap(param1:TextFlowLine) : Number
      {
         ...
         var _loc4_:Number = _loc3_.direction == Direction.LTR ? Number(_loc3_.paragraphStartIndent) : Number(_loc3_.paragraphEndIndent);
         if(_loc3_.direction == Direction.LTR && param1.location & 0)
         {
            ...
            if(param1.hasNumberLine && _loc2_.getParentByType(ListItemElement).computedFormat.listStylePosition == ListStylePosition.INSIDE)
            {
               ...
               _loc4_ += TextFlowLine.getNumberLineInsideLineWidth(_loc6_);
            }
         }
         do
         {
            _loc4_ += this._blockProgression == BlockProgression.TB ? _loc2_.getEffectivePaddingLeft() : _loc2_.getEffectivePaddingTop();
      ...
      private function calculateLineAlignmentAndBounds(param1:TextLine, param2:TextLine, param3:AlignData) : AlignData
      {
         ...
         var _loc4_:Number = !!this._parcelList.explicitLineBreaks ? Number(this.calculateLineWidthExplicit(param1)) : Number(param1.textWidth);
         ...
         if(param3)
         {
            ...
            if(this._blockProgression == BlockProgression.TB)
            {
               if(!this._measuring)
               {
                  ...
                  {
                     _loc7_ = Number(param3.textAlign == TextAlign.CENTER ? Number(_loc9_ / 2) : Number(_loc9_));
         ...
         if(!param3)
         {
            _loc13_ = this._curParaFormat.direction == Direction.LTR ? Number(Math.max(this._curLine.lineOffset,0)) : Number(this._curParaFormat.paragraphEndIndent);
            _loc13_ = this._blockProgression == BlockProgression.RL ? Number(this._curLine.y - _loc13_) : Number(this._curLine.x - _loc13_);
            if(param2)
            {
               _loc14_ = this._blockProgression == BlockProgression.TB ? Number(param2.x + this._curLine.x) : Number(param2.y + this._curLine.y);
               _loc13_ = Math.min(_loc13_,_loc14_);
               if(TextFlowLine.getNumberLineListStylePosition(param2) == ListStylePosition.OUTSIDE)
               {
                  if((_loc15_ = (_loc15_ = _loc14_ + TextFlowLine.getNumberLineInsideLineWidth(param2)) - _loc8_) > 0)
                  {
                     _loc7_ += _loc15_;
                  }
               }
      ...
      protected function calculateLeadingParameters(param1:FlowLeafElement, param2:int, param3:TextLine = null) : Number
      {
         ...
         if(this._curLineLeadingModel == LeadingModel.BOX)
         {
            ...
            this._curLineLeading = !!_loc5_ ? Number(_loc5_.bottom) : Number(0);
            return !!_loc5_ ? Number(-_loc5_.top) : Number(0);
         }
      ...
      protected function finishComposeLine(param1:TextLine, param2:TextLine) : void
      {
         ...
         else
         {
            ...
            if(this._atColumnStart)
            {
               ...
               else if(this._curLineLeadingModel == LeadingModel.APPROXIMATE_TEXT_FIELD)
               {
                  ...
                  if(this._blockProgression == BlockProgression.TB)
                  {
                     _loc3_ = Number(Math.round(_loc4_ + _loc3_) - _loc4_);
                  }
                  else
                  {
                     _loc3_ = Number(_loc4_ - Math.round(_loc4_ - _loc3_));
                  }
                  _loc10_ = 0;
               }
               else
            ...
            if(_loc10_ == BaselineOffset.ASCENT)
            {
               ...
               if(param2)
               {
                  _loc3_ += Math.max(_loc13_,TextFlowLine.getTextLineTypographicAscent(param2,null,0,0));
               }
               else
               {
                  _loc3_ += _loc13_;
            ...
            else if(_loc10_ == BaselineOffset.LINE_HEIGHT)
            {
               ...
               {
                  _loc14_ = !!this._atColumnStart ? true : Boolean(ParagraphElement.useUpLeadingDirection(this._curLineLeadingModel));
                  _loc15_ = this._atColumnStart || this._lastLineLeadingModel == "" ? true : Boolean(ParagraphElement.useUpLeadingDirection(this._lastLineLeadingModel));
                  if(_loc14_)
                  {
         ...
         var _loc7_:Number = this._atColumnStart && this._curLineLeadingModel != LeadingModel.BOX ? 0 : Number(this._curLine.spaceBefore);
         var _loc8_:Number = !!this._atColumnStart ? 0 : Number(this._paragraphSpaceCarried);
      ...
      private function applyTextAlign(param1:Number) : void
      {
         ...
         if(this._blockProgression == BlockProgression.TB)
         {
            for each(_loc5_ in this._alignLines)
            {
               ...
               _loc7_ = _loc5_.textAlign == TextAlign.CENTER ? Number(_loc9_ / 2) : Number(_loc9_);
               ...
               if(_loc3_ && TextFlowLine.getNumberLineListStylePosition(_loc3_) == ListStylePosition.OUTSIDE)
               {
                  ...
                  if((_loc12_ += TextFlowLine.getNumberLineInsideLineWidth(_loc3_)) > this._parcelRight)
                  {
         ...
         else
         {
            for each(_loc5_ in this._alignLines)
            {
               ...
               if(_loc3_ && TextFlowLine.getNumberLineListStylePosition(_loc3_) == ListStylePosition.OUTSIDE)
               {
                  ...
                  if((_loc12_ += TextFlowLine.getNumberLineInsideLineWidth(_loc3_)) > this._parcelBottom)
                  {
      ...
      {
         this._lastLineDescent = this._curLineLeadingModel == LeadingModel.BOX ? Number(this._curLineLeading) : Number(param1.descent);
      ...
      private function clearControllers(param1:ContainerController, param2:ContainerController) : void
      {
         ...
         while(_loc3_ <= _loc4_)
         {
            (_loc5_ = ContainerController(this._flowComposer.getControllerAt(_loc3_))).setContentBounds(0,0,0,0);
      ...
      protected function parcelHasChanged(param1:Parcel) : void
      {
         ...
         if(_loc3_)
         {
            this._verticalSpaceCarried = this._blockProgression == BlockProgression.RL ? Number(_loc3_.getTotalPaddingRight()) : Number(_loc3_.getTotalPaddingTop());
      ...
      {
         var _loc2_:Number = !!param1.measureWidth ? 0 : Number(param1.compositionWidth);
         var _loc3_:Number = param1.horizontalScrollPosition;
         this._controllerVisibleBoundsXTW = Twips.roundTo(this._blockProgression == BlockProgression.RL ? Number(_loc3_ - _loc2_) : Number(_loc3_));
         this._controllerVisibleBoundsYTW = Twips.roundTo(param1.verticalScrollPosition);
         this._controllerVisibleBoundsWidthTW = !!param1.measureWidth ? 0 : int(Twips.to(param1.compositionWidth));
         this._controllerVisibleBoundsHeightTW = !!param1.measureHeight ? 0 : int(Twips.to(param1.compositionHeight));
      }
      ...
      private function getLineAdjustmentForInline(param1:TextLine, param2:String, param3:Boolean) : LeadingAdjustment
      {
         ...
         while(_loc6_ && _loc7_ < this._curLine.absoluteStart + this._curLine.textLength)
         {
            if(_loc7_ >= this._curLine.absoluteStart || _loc7_ + _loc6_.textLength >= this._curLine.absoluteStart)
            {
               ...
               {
                  if((_loc10_ = _loc6_ as InlineGraphicElement).effectiveFloat == Float.NONE && !(this._blockProgression == BlockProgression.RL && _loc6_.parent is TCYElement))
                  {
                     if(_loc9_ < _loc10_.getEffectiveFontSize())
                     {
                        if((_loc9_ = Number(_loc10_.getEffectiveFontSize())) >= _loc8_)
                        {
