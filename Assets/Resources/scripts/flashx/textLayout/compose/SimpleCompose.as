   ...
   public class SimpleCompose extends BaseCompose
   {
      ...
      override protected function composeNextLine() : TextLine
      {
         ...
         if(_listItemElement && _listItemElement.getAbsoluteStart() == _curElementStart + _curElementOffset)
         {
            ...
            _loc1_ = TextFlowLine.createNumberLine(_listItemElement,_curParaElement,_flowComposer.swfContext,!!_loc3_ ? 0 : Number(_parcelList.leftMargin));
      ...
      override protected function finalParcelAdjustment(param1:ContainerController) : void
      {
         ...
         if(!isNaN(_parcelLogicalTop))
         {
            if(_loc5_)
            {
               _loc4_ = Number(_parcelLogicalTop);
            }
            else
            {
               _loc3_ = Number(_parcelLogicalTop);
            }
         }
         if(!_measuring)
         {
            if(_loc5_)
            {
               _loc3_ = Number(_accumulatedMinimumStart);
            }
            else
            {
               _loc2_ = Number(_accumulatedMinimumStart);
            }
         }
         else
         {
            ...
            for each(_loc6_ in this._lines)
            {
               if((_loc17_ = (_loc16_ = param1.textFlow.findLeaf(_loc7_)).getParagraph()) != _loc15_)
               {
                  ...
                  while(_loc20_ && _loc20_.parent)
                  {
                     if(_loc5_)
                     {
                        _loc13_ += _loc20_.getEffectivePaddingRight();
                        _loc14_ += _loc20_.getEffectivePaddingTop();
                     }
                     else
                     {
                        _loc13_ += _loc20_.getEffectivePaddingTop();
                        _loc14_ += _loc20_.getEffectivePaddingLeft();
                     }
               ...
               if(_loc6_.numChildren > 0)
               {
                  ...
                  _loc18_ = Number(TextFlowLine.getTextLineTypographicAscent(_loc6_,_loc16_,_loc21_,_loc7_ + _loc6_.rawTextLength));
               }
               if(null != _loc17_)
               {
                  if((_loc12_ = _loc17_.computedFormat).direction == Direction.LTR)
                  {
                     _loc8_ = Number(Math.max(_loc12_.textIndent,0));
               ...
               _loc9_ += _loc14_;
               _loc10_ = _loc6_.textBlockBeginIndex == 0 ? Number(_loc9_ + _loc8_) : Number(_loc9_);
               _loc10_ = !!_loc5_ ? Number(_loc6_.y - _loc10_) : Number(_loc6_.x - _loc10_);
               if(_loc19_ = TextFlowLine.findNumberLine(_loc6_))
               {
                  _loc22_ = !!_loc5_ ? Number(_loc19_.y + _loc6_.y) : Number(_loc19_.x + _loc6_.x);
                  _loc10_ = Math.min(_loc10_,_loc22_);
               }
               if(_loc5_)
               {
                  _loc3_ = Number(Math.min(_loc10_,_loc3_));
                  _loc4_ = Number(Math.max(_loc6_.x + Math.max(_loc18_,_loc6_.ascent) + _loc13_,_loc4_));
               }
               else
               {
                  _loc2_ = Number(Math.min(_loc10_,_loc2_));
                  if(_loc18_ < _loc6_.ascent)
                  {
                     _loc18_ = Number(_loc6_.ascent);
                  }
                  _loc3_ = Number(Math.min(_loc6_.y - (_loc18_ + _loc13_),_loc3_));
               }
