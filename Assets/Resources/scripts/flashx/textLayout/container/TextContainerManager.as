package flashx.textLayout.container
{
   ...
   public class TextContainerManager extends EventDispatcher implements ISWFContext, IInteractionEventHandler, ISandboxSupport
   {
      ...
      {
         return this._composeState == tlf_internal::COMPOSE_FACTORY ? Boolean(this._damaged) : Boolean(this._textFlow.flowComposer.isDamaged(this._textFlow.textLength));
      }
      ...
      public function setTextFlow(param1:TextFlow) : void
      {
         ...
         this._composeState = param1.interactionManager || param1.mustUseComposer() ? int(tlf_internal::COMPOSE_COMPOSER) : int(tlf_internal::COMPOSE_FACTORY);
      ...
      public function callInContext(param1:Function, param2:Object, param3:Array, param4:Boolean = true) : *
      {
         ...
         if((_loc5_ = param2 as TextBlock) && _expectedFactoryCompose == TextLineFactoryBase._factoryComposer)
         {
      ...
      public function compose() : void
      {
         ...
         else if(this._damaged)
         {
            ...
            if(false)
            {
               while(!((_loc4_ = this._composedLines[0]) == null || _loc4_ is TextLine))
               {
            ...
            _loc2_.swfContext = !true ? {} : this._swfContext;
      ...
      private function addActivationEventListeners() : void
      {
         ...
         if(this._composeState == tlf_internal::COMPOSE_FACTORY)
         {
            ...
            {
               _loc1_ = this._handlersState == tlf_internal::HANDLERS_NOTADDED ? int(tlf_internal::HANDLERS_CREATION) : int(tlf_internal::HANDLERS_ACTIVE);
            }
         }
      ...
      public function drawBackgroundAndSetScrollRect(param1:Number, param2:Number) : Boolean
      {
         ...
         {
            _loc4_ = (_loc9_ = this.getController()).contentWidth;
            _loc5_ = _loc9_.contentHeight;
         }
         if(isNaN(this.compositionWidth))
         {
            _loc6_ = (_loc10_ = this._composeState == tlf_internal::COMPOSE_FACTORY ? Number(this._contentLeft) : Number(_loc9_.contentLeft)) + _loc4_ - param1;
         }
         else
         {
         ...
         {
            _loc7_ = (_loc11_ = this._composeState == tlf_internal::COMPOSE_FACTORY ? Number(this._contentTop) : Number(_loc9_.contentTop)) + _loc5_ - param2;
         }
         else
         {
