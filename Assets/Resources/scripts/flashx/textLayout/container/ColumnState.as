package flashx.textLayout.container
{
   ...
   public class ColumnState
   {
      ...
      tlf_internal function updateInputs(param1:String, param2:String, param3:ContainerController, param4:Number, param5:Number) : void
      {
         ...
         var _loc14_:Boolean = _loc10_.columnCount == FormatValue.AUTO && (_loc10_.columnWidth == FormatValue.AUTO || Number(_loc10_.columnWidth) == 0) || param3.rootElement.computedFormat.lineBreak == LineBreak.EXPLICIT || isNaN(param1 == BlockProgression.RL ? Number(param5) : Number(param4));
      ...
      tlf_internal function computeColumns() : void
      {
         ...
         var _loc4_:Number = this._blockProgression == BlockProgression.RL ? Number(this._compositionHeight) : Number(this._compositionWidth);
         var _loc5_:Number = this._blockProgression == BlockProgression.RL ? Number(this._paddingTop + this._paddingBottom) : Number(this._paddingLeft + this._paddingRight);
         _loc4_ = _loc4_ > _loc5_ && !isNaN(_loc4_) ? Number(_loc4_ - _loc5_) : Number(0);
         ...
         {
            _loc1_ = Number(this._inputColumnGap);
         ...
         if(this._blockProgression == BlockProgression.TB)
         {
            if(this._columnDirection == Direction.LTR)
            {
               ...
               _loc8_ = Number(this._columnWidth + this._columnGap);
               _loc11_ = Number(this._columnWidth);
            }
            else
            {
               _loc6_ = !!isNaN(this._compositionWidth) ? Number(this._paddingLeft) : Number(this._compositionWidth - this._paddingRight - this._columnWidth);
               _loc8_ = Number(-(this._columnWidth + this._columnGap));
               _loc11_ = Number(this._columnWidth);
            }
            ...
            _loc10_ = Number(this._compositionHeight > _loc12_ && !isNaN(this._compositionHeight) ? Number(this._compositionHeight - _loc12_) : Number(0));
         }
         else if(this._blockProgression == BlockProgression.RL)
         {
            _loc6_ = !!isNaN(this._compositionWidth) ? Number(-this._paddingRight) : Number(this._paddingLeft - this._compositionWidth);
            _loc7_ = this._paddingTop;
            _loc8_ = 0;
            _loc9_ = Number(this._columnWidth + this._columnGap);
            _loc12_ = this._paddingLeft + this._paddingRight;
            _loc11_ = Number(this._compositionWidth > _loc12_ ? Number(this._compositionWidth - _loc12_) : Number(0));
            _loc10_ = Number(this._columnWidth);
         }
         if(_loc11_ == 0)
         {
            _loc11_ = 0;
