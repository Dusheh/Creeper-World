package flashx.textLayout.operations
{
   ...
   public class CompositeOperation extends FlowOperation
   {
      ...
      override public function doOperation() : Boolean
      {
         ...
         {
            _loc1_ = _loc1_ && FlowOperation(this._operations[_loc2_]).doOperation();
            _loc2_++;
         }
         return true;
      }
      ...
      override tlf_internal function merge(param1:FlowOperation) : FlowOperation
      {
         ...
         if(param1 is InsertTextOperation || param1 is SplitParagraphOperation || param1 is DeleteTextOperation)
         {
            ...
            _loc3_ = this._operations && this._operations.length ? FlowOperation(this._operations[this._operations.length - 1]) : null;
            if(_loc3_)
            {
               _loc2_ = _loc3_.merge(param1);
            }
            if(_loc2_ && !(_loc2_ is CompositeOperation))
            {
            ...
            setGenerations(beginGeneration,param1.endGeneration);
            return this;
         }
         return null;
      }
